...
module.exports = class Harvester{
    ...
    constructor(httpPort = 8081, webSocketPort = 8082, remoteServerPort = undefined, openBrowser = true, debug = false){
        ...
        app.get('/captcha/:captchaCallbackIndex/:siteKey/:host', (request, response)=>{
            ...
            response.send('' +
                ...
                '       <script type="text/javascript">function captchaLoad() { window.stamp = Date.now(); }</script>' +
                ...
                '           var submit_form = () => { document.getElementById("stamp").value = "" + window.stamp; document.getElementById("captcha_form").submit();};' +
                '           document.addEventListener("DOMContentLoaded", () => setTimeout(() => { randomMouse(Math.floor(Math.random() * 3 + 3)).then(() => grecaptcha.execute()); }));' +
                '       </script>' +
                '   </body>' +
                '</html>'
            );
        ...
        app.post('/captcha', (request, response)=>{
            ...
            captchaCallback({ token: request.body['g-recaptcha-response'], stamp: request.body['stamp'] });
                    ...
                    switch(event){
                        ...
                        case Event.TCP.CaptchaRequestEvent:
                            ...
                            if(!this.firstCaptchaRequested){
                                if(openBrowser){
                                    opn('http://localapi.supremenewyork.com:' + httpPort);
                                }
    ...
    getResponse(host, siteKey, prioritise = false){
        if(!this.firstCaptchaRequested){
            if(this.openBrowser){
                opn('http://localapi.supremenewyork.com:' + this.httpPort);
            }
            this.firstCaptchaRequested = true;
        }
